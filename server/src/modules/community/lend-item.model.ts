...
const lendItemSchema = new Schema({
  ...
  category: { type: String, enum: ["CYCLE","LAPTOP","BOOK","FOOD","ELECTRONICS","OTHER"], default: "OTHER" },
  status: { type: String, enum: ["AVAILABLE","RESERVED","LENT","CLOSED"], default: "AVAILABLE" },
...
const borrowRequestSchema = new Schema({
  ...
  status: { type: String, enum: ["PENDING","APPROVED","REJECTED","RETURNED","CANCELLED"], default: "PENDING" },
  dueAt: Date,
}, { timestamps: true });
